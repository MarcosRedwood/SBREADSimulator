...
class LifeStuff extends Scene {
    ...
    bool trigger(List<Player> playerList) {
        ...
        for (num i = 0; i < dead.length; i++) {
            Player d = dead[i];
            if ((d.aspect.isThisMe(Aspects.OIL) || d.aspect .isThisMe(Aspects.SPICE)) && d.hasPowers()) {
        ...
        for (num i = 0; i < nonGuides.length; i++) {
            ...
            if ((player.aspect.isThisMe(Aspects.OIL) || player.aspect.isThisMe(Aspects.SPICE) && player.hasPowers()) || player.canGhostCommune() != null) {
    ...
    List<List<Player>> findGuidesAndNonGuides() {
        ...
        for (Player possibleGuide in session.getReadOnlyAvailablePlayers()) {
            if (possibleGuide.aspect.isThisMe(Aspects.SPICE) || possibleGuide.aspect.isThisMe(Aspects.OIL) || possibleGuide.canGhostCommune() != null) {
        ...
        for (Player possibleGuide in session.getReadOnlyAvailablePlayers()) {
            ...
            } else if (!possibleGuide.aspect.isThisMe(Aspects.SPICE) && !possibleGuide.aspect.isThisMe(Aspects.OIL) || possibleGuide.canGhostCommune() == null) {
    ...
    void renderContent(Element div) {
        ...
        for (num i = 0; i < this.enablingPlayerPairs.length; i++) {
            ...
            if(enablingAspect != Aspects.SPICE) enablingAspect = Aspects.OIL;
    ...
    CanvasElement drainDeadForReviveSelf(Element div, String str, Player player, SBURBClass className, Aspect enablingAspect) {
        ...
        if (ghost != null && ghost.causeOfDrain != "" && myGhost != ghost) {
            ...
            } else if (enablingAspect.isThisMe(Aspects.SPICE) || player.prophecy == ProphecyState.FULLFILLED ) {
