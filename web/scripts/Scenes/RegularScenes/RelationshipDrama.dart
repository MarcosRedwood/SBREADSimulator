	...
	void confessTooManyFeeling(Element div, Player player, Player crush){
		...
		if(r2.saved_type == r2.goodBig){
			...
			if(player2.grimDark > 3){
				...
				if(InterestManager.ROMANTIC.playerLikes(player1) || player1.aspect == Aspects.HONEY){
					chatText += Scene.chatLine(player1Start, player1,"Fuck. I'm sorry I didn't keep it to myself. ");
					if(InterestManager.ROMANTIC.playerLikes(player2)|| player2.aspect == Aspects.HONEY || player2.aspect == Aspects.ROCK){
						chatText += Scene.chatLine(player2Start, player2,"Better than keeping it bottled up. ");
					}
					player1.addStat(Stats.SANITY, -10);
				}else if(player2.class_name == SBURBClassManager.BARD || player2.aspect == Aspects.ROCK || InterestManager.TERRIBLE.playerLikes(player2)){
					chatText += Scene.chatLine(player1Start, player1,"Fuck. Why don't I ever learn that people suck?");
					if(player2.class_name == SBURBClassManager.SEER || player2.aspect == Aspects.BREAD || InterestManager.ROMANTIC.playerLikes(player2)){
					...
					player1.addStat(Stats.SANITY, -10);
				}else if(player2.class_name == SBURBClassManager.PAGE || player2.aspect == Aspects.BREAD || InterestManager.ACADEMIC.playerLikes(player2)){
					chatText += Scene.chatLine(player1Start, player1,"But... I was even brave and told you and everything...");
					if(player2.class_name == SBURBClassManager.KNIGHT || player2.aspect == Aspects.ROCK || InterestManager.ACADEMIC.playerLikes(player2)){
	...
	void confessFeelings(Element div, Player player, Player crush){
		...
		if(r2.saved_type == r2.goodBig){
			...
			if(player2.grimDark  > 3){
				...
				}else if(player1.class_name == SBURBClassManager.PAGE || player1.aspect == Aspects.BREAD || InterestManager.ACADEMIC.playerLikes(player1)){
					chatText += Scene.chatLine(player1Start, player1,"But... I was even brave and told you and everything...");
					if(player2.class_name == SBURBClassManager.KNIGHT || player2.aspect == Aspects.ROCK || InterestManager.CULTURE.playerLikes(player2)){
     ...
     ////session.logger.info("Crush: " + crush.title()) //these are occasionally the same despite my best efforts
		...
		if(crush.dead == true){
			...
			if(r2crush != null && r2crush.saved_type == r2crush.goodBig){ //best friend has a crush on them, too.
				...
				if(player2.class_name == SBURBClassManager.PAGE || player2.class_name == SBURBClassManager.MAID || player2.class_name == SBURBClassManager.SYLPH || player2.aspect == Aspects.BREAD){
				...
				}else if(player2.class_name == SBURBClassManager.KNIGHT || player2.class_name == SBURBClassManager.SEER || player2.class_name == SBURBClassManager.HEIR || player2.aspect == Aspects.HONEY){ //fight player1
