...
class FreeWillStuff extends Scene {
    ...
    bool trigger(List<Player> playerList) {
        ...
        for (num i = 0; i < players.length; i++) {
            ...
            if ((player.land == null || (player.land != null && player.hasPowers()) || player.canMindControl() != null) && player.getStat(Stats.FREE_WILL)>0) { // you need powers AND free will.
    ...
    dynamic considerDisEngagingMurderMode(Player player) {
        if (player.murderMode) {
            ...
            var spacePlayerEnemy = findAspectPlayer(enemies, Aspects.SPEAR);
    ...
    bool isValidTargets(List<Player> enemies, Player player) {
        Player spacePlayerEnemy = findAspectPlayer(enemies, Aspects.SPEAR);
    ...
    Player findMurderModePlayerBesides(Player player) {
        ...
        if (ret == player && player.aspect != Aspects.KNIFE) ret = null; //you should TOTALLY be able to calm your past selves shit.
        return ret;
    }
    ...
    Player findNonGodTierBesidesMe(Player player) {
        ...
        if (player.aspect.isThisMe(Aspects.KNIFE) && !player.godTier) return player; //god tier yourself first.
        ...
        if (ret.target == player && player.aspect != Aspects.KNIFE) ret = null;
    ...
    List<dynamic> findBestPatsy(Player player, List<Player> enemies) {
        ...
        for (num i = 0; i < living.length; i++) {
            var p = living[i];
            if (p != player || player.aspect.isThisMe(Aspects.KNIFE)) { //can't be own patsy
                if (bestPatsy == null) {
    ...
    bool canInfluenceEnemies(Player player) {
        if (player.aspect.isThisMe(Aspects.BREAD) || player.aspect.isThisMe(Aspects.MILK) || player.aspect.isThisMe(Aspects.HONEY)) {
        ...
        if (player.aspect.isThisMe(Aspects.ROCK)) {
    ...
    bool canAlterNegativeFate(Player player) {
        if (player.aspect .isThisMe(Aspects.BLAZE) || player.aspect.isThisMe(Aspects.OIL) || player.aspect.isThisMe(Aspects.MILK) || player.aspect.isThisMe(Aspects.HONEY)) {
            if (player.class_name == SBURBClassManager.MAID || player.class_name == SBURBClassManager.SEER) {
                return true;
            }
        }

        if (player.aspect.isThisMe(Aspects.SPICE)) {
    ...
    String getManipulatableTrait(Player player) {
        String ret = "???";
        if (player.aspect.isThisMe(Aspects.MILK)) ret = "identity";
        if (player.aspect.isThisMe(Aspects.BREAD)) ret = "relationships";
        if (player.aspect.isThisMe(Aspects.HONEY)) ret = "mind";
        if (player.aspect.isThisMe(Aspects.ROCK)) ret = "sanity";
        if (player.aspect.isThisMe(Aspects.FRUIT)) ret = "beliefs";
        if (player.aspect.isThisMe(Aspects.SPICE)) ret = "fear";
        if (player.aspect.isThisMe(Aspects.WINE)) ret = "motivation";
        if (player.aspect.isThisMe(Aspects.SPEAR)) ret = "commitment";
        if (player.aspect.isThisMe(Aspects.KNIFE)) ret = "fate";
        if (player.aspect.isThisMe(Aspects.BLAZE)) ret = "luck";
        if (player.aspect.isThisMe(Aspects.FLOOD)) ret = "nothing";
        if (player.aspect.isThisMe(Aspects.OIL)) ret = "purpose";
        return ret;
    }

    String getInfluenceSymbol(player) {
        if (player.aspect.isThisMe(Aspects.HONEY)) return "mind_forehead.png";
        if (player.aspect.isThisMe(Aspects.ROCK)) return "rage_forehead.png";
        if (player.aspect.isThisMe(Aspects.BREAD)) return "blood_forehead.png";
        if (player.aspect.isThisMe(Aspects.MILK)) return "heart_forehead.png";
        return null;
    }
        ...
        if (this.isValidTargets(enemies, player) && patsy != null) {
            if (patsyVal > enemies.length / 2 && patsy.getStat(Stats.SANITY) < 1) {
               // session.logger.info("manipulating someone to go into murdermode patsyVal = $patsyVal");
                ...
                } else if (player.aspect.isThisMe(Aspects.KNIFE) && rand.nextDouble() > .25) { //most manipulative time bastards are from teh future
    ...
    String forceSomeOneElseBecomeGod(Player player) {
        ...
        if (sacrifice != null && !sacrifice.dead && !sacrifice.godTier) {
            ...
            } else if (player.aspect.isThisMe(Aspects.KNIFE) && rand.nextDouble() > .25) {
                timeIntro = " from the future";
            }
            ...
            if (sacrifice.getStat(Stats.FREE_WILL) <= player.getStat(Stats.FREE_WILL) && player.getStat(Stats.POWER) < 200) { //can just talk them into this terrible idea.   not a good chance of working.
                if (sacrifice.godDestiny) { //if my dream self is dead and i am my real self....
                    ...
                    if (!sacrifice.godDestiny) {
                        ret += sacrifice.makeDead("trying to go God Tier against destiny.", player);
        ...
        if (!player.godTier) {
            ...
            if (player.godDestiny) {
                ...
                if (player.rollForLuck() > 100) {
                    ...
                    session.removeAvailablePlayer(player);
                    intro += player.makeDead("trying to go God Tier against destiny.",player); //if slab, no corpse produced.
                    this.renderPlayer1 = player;
    ...
    String godTierHappens(Player player) {
        String ret = "";
        if (!player.isDreamSelf && player.hasLand() && player.hasMoon()) {
            ret += "The " + player.htmlTitleBasic() + "'s body glows, and rises Skaiaward. " + "On ${player.moonName}, their dream self takes over and gets a sweet new outfit to boot.  ";
            this.session.stats.questBed = true;
            ret += player.makeDead("on their quest bed",player); //doesn't actually matter, they can loot their own corpse
        } else {
    ...
    String killMurderer(Player player, Player murderer) {
        if (player.getStat(Stats.POWER) > murderer.getStat(Stats.POWER)) {
            if (player.getStat(Stats.POWER) * player.getPVPModifier("Attacker") > murderer.getStat(Stats.POWER) * murderer.getPVPModifier("Defender")) { //power is generic. generally scales with any aplicable stats. lets me compare two different aspect players.
                ...
                this.renderPlayer1 = player;
                this.renderPlayer2 = murderer;
                String ret = murderer.makeDead("being put down like a rabid dog by the " + player.titleBasic(),player);
                player.pvpKillCount ++;
                ...
                session.removeAvailablePlayer(murderer);
                session.removeAvailablePlayer(player);
                String ret = player.makeDead("fighting against the crazy " + murderer.titleBasic(), murderer);
    ...
    String considerMakingEctobiologistDoJob(Player player) {
        if (!this.session.stats.ectoBiologyStarted && player.gnosis > 0 && player.grimDark < 2) {
            String timeIntro = "";
            if (player.aspect.isThisMe(Aspects.KNIFE) && rand.nextDouble() > .25) {
                timeIntro = " from the future";
            }
            if (player.leader) {
    ...
    String considerMakingSpacePlayerDoJob(Player player) {
        Player space = findAspectPlayer(availablePlayers, Aspects.SPEAR);
