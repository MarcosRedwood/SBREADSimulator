...
class MurderPlayers extends Scene {
	...
	dynamic contentForRender(Element div){
		...
		for(num i = 0; i<this.murderers.length; i++){
			...
			if(worstEnemy !=null && worstEnemy.dead == false && this.canCatch(m,worstEnemy)){
				...
				if(worstEnemy.aspect.isThisMe(Aspects.BREAD) && worstEnemy.getStat(Stats.POWER) > 25){
				...
				if(m.getStat(Stats.SANITY)>0){
					...
					if(!m.dead && worstEnemy != null && !this.canCatch(m,worstEnemy)){
						...
						}else if(worstEnemy.aspect.isThisMe(Aspects.FLOOD)){
						...
						}else if (worstEnemy.aspect.isThisMe(Aspects.SPEAR)){
	...
	bool canCatch(Player m, Player worstEnemy){
		...
		if(worstEnemy.aspect.isThisMe(Aspects.FLOOD) && worstEnemy.isVoidAvailable() && worstEnemy.getStat(Stats.POWER) >50) return false;
		if(worstEnemy.aspect.isThisMe(Aspects.SPEAR) && worstEnemy.getStat(Stats.POWER) > 50){
