...
class Reckoning extends Scene {
  ...
  String renderContent(Element div) {
    ...
    String intro = "";
    if (this.session.battlefield.blackKing.getStat(Stats.CURRENT_HEALTH) < 0) {
    ...
    if (this.session.stats.ectoBiologyStarted) {
    ...
    } else if (!this.session.stats.ectoBiologyStarted && leader.aspect == Aspects.KNIFE && !leader.dead) {
      ...
      if (findAspectPlayer(session.players, Aspects.KNIFE).doomedTimeClones.length > 0) {
