...
class RedMiles extends Scene {
  ...
  void destroyLand(Element div, [int count=0]) {
      ...
      for(Player p in session.players) {
        if(!p.aspect.isThisMe(Aspects.SPEAR) && p.land != null  && !p.land.dead) targets.add(p.land);
        if(p.aspect.isThisMe(Aspects.SPEAR) && p.land != null  && !p.land.dead) spaceTargets.add(p.land);
